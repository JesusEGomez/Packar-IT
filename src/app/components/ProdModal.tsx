    ...
    <div className="flex flex-col p-4">
      ...
      <div className="flex gap-x-4">
        {array.map((item, index) => (
          <button
            onClick={() => openSelectProduct(item.size)}
            className="border-2 relative border-slate-300 rounded p-3"
            key={index}
          >
            <div
      ...
      {specialProduct && (
        <div className="fixed top-0 z-20 left-0 right-0 bottom-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-xl pr-4 pl-4 pt-16 pb-16 h-screen overflow-y-auto scrollbar-hidden">
            <SpecialProdModal
              closeModal={closeSelectProduct}
              justClose={justClose}
              closeFirstModal={close}
