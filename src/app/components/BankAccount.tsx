...
const BankAccount = (props: any) => {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      const data = await fetch(
        "https://restcountries.com/v3.1/region/europe?fields=cca2,idd,flag,name"
      );
  ...
  return (
    <form
      className="flex flex-col items-center p-2 my-4 overflow-y-scroll"
          ...
          >
            ...
            {countries &&
              countries.map((country) => (
                ...
                >{`${country.flag}${country.idd.root}${country.idd.suffixes[0]} (${country.name.common})`}</option>
              ))}
          </select>
          <input
            {...register("phone", {
