  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    ...
    if (data) {
      ...
      try {
        ...
        createAccount.ok ||
          (createAccount.status === 504 && setCuentaEnviada(true));
        createAccount.ok ||
          (createAccount.status === 504 &&
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      const response = await fetch(
        `/api/auth/getProfileById/?id=${parsedUser._id}`
      );
    ...
    const fetchCountries = async () => {
      const data = await fetch(
        "https://restcountries.com/v3.1/region/europe?fields=cca2,idd,flag,name,translations"
      );
    ...
    const fetchAccount = async () => {
      ...
      const info = await data.json();
      const totalDifference = info.reduce(
        (
          total: number,
          transfer: { amount: number; application_fee_amount: number }
        ) => {
    ...
    >
      ...
      {profile?.account.state === "loaded" ? (
        <div className="flex flex-col h-max p-3 gap-y-3 justify-center">
          <h1 className="text-xl mx-2 my-4">
            Tu cuenta está en proceso de revision
          </h1>
        ...
        <div className="flex flex-col h-max p-3 gap-y-3 justify-center">
          <h1 className="text-xl mx-2 my-4">
            Tu total acumulado para esta semana es de:
          </h1>
        ...
        <div>
          <h1 className="text-xl my-4 pb-4 border-b">
            Ingresa los datos de tu cuenta para recibir tus pagos
          </h1>
          <select
            {...register("countries", {
            ...
            {countries &&
              countries.map((country, i) => (
                <option
                  value={country.cca2}
                  key={i}
                >{`${country.translations.spa.common} (${country.cca2})`}</option>
              ))}
          </select>
              ...
              >
                ...
                {countries &&
                  countries.map((country, i) => (
                    ...
                    >{`${country.flag} ${country.translations.spa.common} (${country.idd.root}${country.idd.suffixes[0]})`}</option>
                  ))}
              </select>
              <input
                {...register("phone", {
          ...
          <div className="flex flex-col w-full gap-y-2">
            ...
            <input
              ...
              type="text"
              id="address"
              placeholder="Carrer de la Ciutat"
              className={`p-2 rounded bg-white w-full ${
                errors.bank ? "border-red-500" : ""
              }`}
            />
          ...
          <div className="flex flex-col w-full gap-y-2">
            ...
            <div className="flex gap-x-3">
              <input
                ...
                id="dd"
                placeholder="DD"
                className={`p-2 rounded bg-white w-20 ${
                  errors.bank ? "border-red-500" : ""
                }`}
              />
              <input
